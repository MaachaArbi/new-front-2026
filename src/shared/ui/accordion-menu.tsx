  ...
  const initialNestedStates = React.useMemo(() => {
    ...
    if (trimmedChain.length > 0) {
      if (props.type === 'multiple') {
        ...
        // Gardes explicites pour noUncheckedIndexedAccess (ADR-F05) : les indices
        // sont garantis valides par le contexte, mais TS ne le prouve pas seul.
        const first = trimmedChain[0]
        if (first !== undefined) {
          mapping['root'] = first
        }
